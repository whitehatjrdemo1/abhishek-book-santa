...
export default class MyDonationScreen extends Component {
    ...
    sendNotifications = (bookDetails, requestStatus) => {
        ...
        db.collection("notifications").where("request_id", "==", requestID).get()
            .then((snapShot) => {
                snapShot.forEach((doc) => {
                    var message = ""
                    if (requestStatus === "bookSent") {
                        ...
                        message = this.state.donor + "Has Shown Interest"
                    }
    ...
    renderItem = ({ item, i }) => (
        <ListItem key={i} title={item.book_name}
            ...
            rightElement={<TouchableOpacity style={[styles.button, { backgroundColor: item.request_status === "Book Sent" ? "green" : "#ff5722" }]} onPress={() => {
                this.sendBook(item)
            }} >
